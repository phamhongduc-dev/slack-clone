...
export const getInfoById = query({
  ...
  handler: async (ctx, argv) => {
    ...
    const member = await ctx.db
      .query('members')
      .withIndex('by_workspacec_id_user_id', (q) => q.eq('workspaceId', argv.id).eq('userId', userId))
      .unique()

    const workspace = await ctx.db.get(argv.id)
...
export const getById = query({
  ...
  handler: async (ctx, argv) => {
    ...
    return await ctx.db.get(argv.id)
  }
})
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const member = await ctx.db
      .query('members')
      .withIndex('by_workspacec_id_user_id', (q) => q.eq('workspaceId', args.id).eq('userId', userId))
      .unique()
